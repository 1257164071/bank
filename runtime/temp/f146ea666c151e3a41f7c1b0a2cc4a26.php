<?php if (!defined('THINK_PATH')) exit(); /*a:5:{s:52:"F:\www\bank.cn/app/admin\view\action\editaction.html";i:1497693403;s:39:"F:\www\bank.cn/app/admin\view\base.html";i:1507780640;s:41:"F:\www\bank.cn/app/admin\view\header.html";i:1509438471;s:39:"F:\www\bank.cn/app/admin\view\menu.html";i:1511338586;s:41:"F:\www\bank.cn/app/admin\view\footer.html";i:1495788367;}*/ ?>
<!DOCTYPE HTML>
<html lang="zh-CN">
    <head>
        <meta charset="utf-8">
...
<section class="Hui-article-box">
    ...
    <div class="Hui-article">
        ...
        <article class="cl pd-20">
            <p class="f-20 text-success"><?php if(request()->action()  == 'addaction'): ?>新增<?php else: ?>编辑<?php endif; ?>行为</p>

            <!-- 表单 -->
            <form id="form" action="<?php echo url('saveAction'); ?>" method="post" class="form form-horizontal">
                <div class="row cl">
                    <label class="form-label col-xs-2 col-sm-2">行为标识<span class="check-tips">（输入行为标识 英文字母）</span></label>
                    <div class="formControls col-xs-10 col-sm-10 ">
                        <input type="text" class="input-text" name="name" value="<?php echo $data['name']; ?>">
                    </div>
                </div>
                <div class="row cl">
                    <label class="form-label col-xs-2 col-sm-2">行为名称<span class="check-tips">（输入行为名称）</span></label>
                    <div class="formControls col-xs-10 col-sm-10 ">
                        <input type="text" class="input-text" name="title" value="<?php echo $data['title']; ?>">
                    </div>
                </div>
                <div class="row cl">
                    <label class="form-label col-xs-2 col-sm-2">行为类型<span class="check-tips">（选择行为类型）</span></label>
                    <div class="formControls col-xs-10 col-sm-10 ">
                ...
                <div class="row cl">
                    <label class="form-label col-xs-2 col-sm-2">行为描述<span class="check-tips">（输入行为描述）</span></label>
                    <div class="formControls col-xs-10 col-sm-10 ">
                        <textarea name="remark" class="textarea"><?php echo $data['remark']; ?></textarea>
                    </div>
                </div>
                <div class="row cl">
                    <label class="form-label col-xs-2 col-sm-2">行为规则<span class="check-tips">（输入行为规则，不写则只记录日志）</span></label>
                    <div class="formControls col-xs-10 col-sm-10 ">
                        <textarea name="rule" class="textarea"><?php echo $data['rule']; ?></textarea>
                    </div>
                </div>
                <div class="row cl">
                    <label class="form-label col-xs-2 col-sm-2">日志规则<span class="check-tips">（记录日志备注时按此规则来生成，支持[变量|函数]。目前变量有：user,time,model,record,data）</span></label>
                    <div class="formControls col-xs-10 col-sm-10 ">
                        <textarea name="log" class="textarea"><?php echo $data['log']; ?></textarea>
                    </div>
                </div>

                <div class="form-item">
                    <?php if(isset($data['id'])): ?><input type="hidden" name="id" value="<?php echo $data['id']; ?>"/><?php endif; ?>
                    <button type="submit" class="btn btn-primary" target-form="form-horizontal">确 定</button>
