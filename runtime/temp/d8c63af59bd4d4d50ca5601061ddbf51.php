<?php if (!defined('THINK_PATH')) exit(); /*a:5:{s:57:"D:\Xampps\htdocs\bank/app/admin\view\admin\nodelists.html";i:1498812222;s:46:"D:\Xampps\htdocs\bank/app/admin\view\base.html";i:1497005003;s:48:"D:\Xampps\htdocs\bank/app/admin\view\header.html";i:1498812223;s:46:"D:\Xampps\htdocs\bank/app/admin\view\menu.html";i:1499140648;s:48:"D:\Xampps\htdocs\bank/app/admin\view\footer.html";i:1495788367;}*/ ?>
<!DOCTYPE HTML>
<html lang="zh-CN">
    <head>
        <meta charset="utf-8">
...
<section class="Hui-article-box">
	...
	<div class="Hui-article">
		<article class="cl pd-20">
			...
			<span class="l">
			<a href="javascript:;" onclick="admin_permission_add('添加应用','<?php echo url('admin/admin/nodeadd'); ?>','500','370')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加应用</a>
			</span> 
		    </div>
		    <div class="app">
			    <?php if(is_array($nodelist) || $nodelist instanceof \think\Collection || $nodelist instanceof \think\Paginator): $i = 0; $__LIST__ = $nodelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$node): $mod = ($i % 2 );++$i;?>
				    <p style="padding: 10px 0 0 10px;"><strong style="color:#f00;"><?php echo $node['title']; ?></strong>
				    <a href="javascript:;" class="btn btn-primary  radius" onclick="admin_permission_add('添加控制器','<?php echo url('admin/admin/nodeadd',array('pid'=>$node['id'],'level'=>2)); ?>','500','370')">添加控制器</a>
				    <a href="javascript:;" class="btn btn-success  radius" onclick="msg_edit('编辑控制器','<?php echo url('admin/admin/nodeedit',array('id'=>$node['id'],'level'=>1)); ?>','500','370')">编辑</a>
				    <a href="javascript:;" onclick="msg_del(this,'<?php echo $node['id']; ?>')" class="btn btn-danger radius">删除</a>
					</p>
				     <?php if(is_array($node['child']) || $node['child'] instanceof \think\Collection || $node['child'] instanceof \think\Paginator): $i = 0; $__LIST__ = $node['child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$action): $mod = ($i % 2 );++$i;?>
				     <dl>
				        <dt>
				        <span><strong><?php echo $action['title']; ?></strong></span>
				        	<a class="btn btn-primary size-S radius" style="text-decoration:none" href="javascript:;" onclick="admin_permission_add('添加方法','<?php echo url('admin/admin/nodeadd',array('pid'=>$action['id'],'level'=>3)); ?>','500','370')">添加方法</a>
				        	<a class="btn btn-success size-S radius"  href="javascript:;" onclick="msg_edit('编辑控制器','<?php echo url('admin/admin/nodeedit',array('id'=>$action['id'],'level'=>2)); ?>','500','370')">编辑</a>
							<a class="btn btn-danger size-S radius" href="javascript:;" onclick="msg_del(this,'<?php echo $action['id']; ?>')" >删除</a>
				        </dt>
				     	<?php if(is_array($action['child']) || $action['child'] instanceof \think\Collection || $action['child'] instanceof \think\Paginator): $i = 0; $__LIST__ = $action['child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$method): $mod = ($i % 2 );++$i;?>
				     		<dd>
				     			<span style="vertical-align: middle"><?php echo $method['title']; ?></span>
								<a href="javascript:;" class="btn btn-success size-MINI radius" onclick="msg_edit('编辑方法','<?php echo url('admin/admin/nodeedit',array('id'=>$method['id'],'level'=>3)); ?>','500','370')" >编辑</a>
								<a href="javascript:;" class="btn btn-danger size-MINI radius" onclick="msg_del(this,'<?php echo $method['id']; ?>')" >删除</a>
				     		</dd>
				     	<?php endforeach; endif; else: echo "" ;endif; ?>
				     </dl>
...
function msg_del(obj,id){
	layer.confirm('删除须谨慎，确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: "<?php echo url('admin/admin/nodedel'); ?>",
			dataType: 'json',
			data:'id='+id,
			success: function(data){
				if(data.code==0){
					...
					window.location.href = "<?php echo url('admin/admin/nodelists'); ?>";
				}else{
					layer.msg(data.msg);
				}
			},
