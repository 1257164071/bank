...
class Dataconfig extends Base
{

    /*配置列表*/
    public function configlist(){
        //dump('asdf');exit();
        $res = db('dataconfig')->select();
        $this->assign("dataconfig",$res);
        return $this->fetch();
    }
    ...
    public function configadd(){
        if (request()->isGet()){
            if (input('param.id')){
                $id = input('param.id');
                $findres = db('dataconfig')->where('id',$id)->find();
                $this->assign('client',$findres);
            }
        }
        ...
        if(request()->isPost()){
            ...
            $data['value'] = trim(input('value'));
            if (empty($data['code'])||empty($data['name'])||empty($data['value'])){
                api_error('字段不能为空！');
            }
            $data['id'] = input('id');
            if(!empty($data['id'])){
                $res = db('dataconfig')->update($data);
            }else{
                $res = db('dataconfig')->insert($data);
            }
            if($res){
                api_success('OK！');
            }else{
                api_error('保存失败！请检查小区是否已存在');
            }
        }
    ...
    public function import($time = 0, $part = null, $start = null) {
        if (is_numeric($time) && is_null($part) && is_null($start)) {
            ...
            $db = new Databases($list[$part], array('path' => realpath(config('data_backup_path')) . DIRECTORY_SEPARATOR, 'compress' => $list[$part][2]));
    ...
    public function bankadd(){
        ...
        if(request()->isPost())
        {
            ...
            $Bank = new Bank();
            $banklevel = $Bank->banklevel();
            $this->assign('banklevel',$banklevel);
            $this->assign("url",url('admin/dataconfig/bankadd'));
            return $this->fetch();
        }
    }
    ...
    public function bankedit(){
        if(request()->isPost())
        {
            ...
            $this->assign('msg', $msg);
            $this->assign("url",url('admin/dataconfig/bankedit'));
